...
window.getProducts = async function() {
  ...
  try {
    ...
    for (let i = 0; i < productCount; i++) {
      ...
      products.push({
        name: product[0],
        priceInEth: web3.utils.fromWei(product[1], 'ether'),
        quantity: product[2]
      });
    }
...
window.addToCartToBlockchain = async function(name, priceInEth, quantity) {
  if (!contract) {
    console.error('Smart contract not initialized');
    return;
  }
  try {
    ...
    await contract.methods.addToCart(name, web3.utils.toWei(priceInEth.toString(), 'ether'), quantity)
      .send({ from: account });
